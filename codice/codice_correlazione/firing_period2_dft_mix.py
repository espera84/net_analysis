...
only_interneurons=False#True#
save_neu_fig=True#False#
n_neuroni=288027
...
results_path="./emix_interval_"+str(t_initial_analysis)+"_"+str(t_final_analysis)+"interval_dim_"+str(interval_dim)+"_fft/"
try:
...
stdev=np.zeros((n_neuroni),dtype=float)
m=np.zeros((n_neuroni),dtype=float)
coef_var=np.zeros((n_neuroni),dtype=float)
coef_var_bet_periods=np.ones((n_neuroni),dtype=float)*10000
fr=np.zeros((n_neuroni),dtype=float)
rad=np.zeros((n_neuroni),dtype=float)
err_with_fr=np.ones((n_neuroni),dtype=float)*10000
periods=np.zeros((n_neuroni),dtype=int)
dist_tot_norm=np.ones((n_neuroni),dtype=float)*10000
perc_amp=np.zeros((n_neuroni),dtype=float)
...
# id_neu=261853
# ou=np.cumsum(n_spk_for_neuron[id_neu,:])
# hist=ou[10:]-ou[:-10]
# plt.figure()
# plt.plot(hist)
######
...
if only_interneurons:
    first_neuron=N_pyr
    last_neuron=n_neuroni
else:
    first_neuron = 0
    last_neuron = 1000#n_neuroni
try:
    with open(results_path + 'phase_and_freq_neurons_from_' + str(first_neuron) + '_' + str(
            last_neuron) + '_' + sigla + '_' + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(
            interval_dim) + '.pkl', 'rb') as f:  # Python 3: open(..., 'wb')
        [fr, rad,neurons_selected,perc_amp] = pickle.load(f)
except:
    amplitude=[]
    for id_neu in range(first_neuron,last_neuron):#n_neuroni):
        stdev[id_neu] = np.std(n_spk_for_neuron[id_neu, :])
        m[id_neu]=np.mean(n_spk_for_neuron[id_neu, :])
        coef_var[id_neu]=stdev[id_neu]/m[id_neu]
        if coef_var[id_neu]>1:
            #hist=n_spk_for_neuron[id_neu,:]
            ou = np.cumsum(n_spk_for_neuron[id_neu, :])
            hist = ou[10:] - ou[:-10]
            [f, a, p] = fourier_analysis(hist)

            fr[id_neu]=f[np.argmax(a[1:])+1]
            periods[id_neu]=round(1/fr[id_neu]*1000)
            ip_fase = np.zeros(periods[id_neu])
            for i in range(1, periods[id_neu]):
                j = 0
                s = []
                while ((i + j * periods[id_neu]) < hist.__len__()):
                    s.append(hist[i + j * periods[id_neu]] - hist[i + j * periods[id_neu] - 1])

                    j = j + 1
                ip_fase[i] = np.sum(s) / j

            ns = []
            j = 0
            while (((j + 1) * periods[id_neu]) < hist.__len__()):
                ns.append(hist[j * periods[id_neu]:(j + 1) * periods[id_neu]].sum())

                j = j + 1
            coef_var_bet_periods[id_neu] = np.std(ns) / np.mean(ns)

            rad[id_neu] = (time[ip_fase.argmax()] - t_initial_analysis) / (1000 / fr[id_neu]) * 2 * np.pi  # *360
            ...
            #rad[id_neu]=p[np.argmax(a[1:])+1]
            pars0=[fr[id_neu],np.pi/2-rad[id_neu]]
            period=1/fr[id_neu]
            #
            # ns = []
            # j = 0
            # while (((j + 1) * period) < hist.__len__()):
            #     ns.append(hist[j * period:(j + 1) * period].sum())
            #
            #     j = j + 1
            #[id_neu] = 0.2#np.std(ns) / np.mean(ns)


            if np.logical_and(err_with_fr[id_neu]>1.,coef_var_bet_periods[id_neu]<0.5):#0.25: #devi trovare una misura migliore per stabilire quali sono buoni fitting
                neurons_selected.append(id_neu)
                perc_amp[id_neu] = a[a[1:].argsort()[-1]] / a[1:].sum()
                amplitude.append(a)
                if save_neu_fig:
                    plt.figure(id_neu)
                    ...
                    plt.plot(time[:hist.__len__()],hist)
                    plt.savefig(results_path+"fit_neu "+str(id_neu)+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+"_"+sigla+".png")
                    plt.close(id_neu)


    with open(results_path +'phase_and_freq_neurons_from_'+str(first_neuron)+'_'+str(last_neuron)+'_'+sigla+'_'+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+'.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([fr,rad,neurons_selected,perc_amp], f)
#plt.figure()
#plt.hist(fr[fr>0],30)
#plt.savefig("hist_fr_"+sigla+'_'+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".png")
plt.figure()
counts, bins =np.histogram(fr[neurons_selected], 30)# np.histogram(fr[fr>0],30)
plt.stairs(counts, bins)
if only_interneurons:
    plt.savefig(results_path + "interneurons_hist_fr_" + sigla + '_' + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(interval_dim) + ".png")

else:
    plt.savefig(results_path+"hist_fr_"+sigla+'_'+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".png")

plt.figure()
plt.scatter(fr[neurons_selected],rad[neurons_selected])#fr[fr>0],rad[fr>0])
if only_interneurons:
    plt.savefig(results_path + "interneurons_scatter_fr_rad_" + sigla + '_' + str(t_initial_analysis) + '_' + str(
        t_final_analysis) + '_' + str(interval_dim) + ".png")
else:
    plt.savefig(results_path+"scatter_fr_rad_"+sigla+'_'+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".png")
...
for i in range(1,bins.__len__()):
    ...
    #neuron_to_plt2=np.in1d(posizioni_neuroni[:,0],neurons_selected)
    neuron_to_plt=np.in1d(posizioni_neuroni[:,0],np.array(neurons_selected)[np.in1d(neurons_selected, np.where(np.logical_and(fr > bins[i - 1], fr < bins[i])))])
...
if only_interneurons:
    fig3.write_html(results_path + "interneurons_frequency_map_" + sigla + "_" + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(interval_dim) + ".html")

else:
    fig3.write_html(results_path+"frequency_map_"+sigla+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".html")
...
#     )
#
# fig3.write_html(results_path+"frequency_map_with_phase_color_"+sigla+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".html")
#plt.ion()
plt.figure()
...
#                       name='frequency ' + str(bins[i-1])+' '+str(bins[i]),
#                       mode='markers',
#                       text=rad[np.array(neurons_selected)[np.in1d(neurons_selected, np.where(np.logical_and(fr > bins[i - 1], fr < bins[i])))]].astype('str'),
#                       marker=dict(size=4,
#                                   color=rad[np.where(np.logical_and(fr>bins[i-1],fr<bins[i]))],#color_discrete_sequence[i],#px.colors.qualitative.Antique[i],#l,#list(colori),#px.colors.qualitative.Antique[i],#                                       color=px.colors.qualitative.Plotly[i],#float(i / n_comp_connesse),,
#                                   showscale=(i==1),
#                                   opacity=1,
...
# fig3.show()
# if only_interneurons:
#     fig3.write_html(results_path + "interneurons_frequency_map_with_phase_color_" + sigla + "_" + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(interval_dim) + ".html")
#
# else:
#     fig3.write_html(results_path+"frequency_map_with_phase_color_"+sigla+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+".html")
...
fig3.show()
if only_interneurons:
    fig3.write_html(results_path + "interneurons_frequency_map_with_phase_color_" + sigla + "_" + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(interval_dim) + "_id.html")
else:
    fig3.write_html(results_path+"frequency_map_with_phase_color_"+sigla+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+"_id.html")
...
fig3.show()
if only_interneurons:
    fig3.write_html(results_path + "interneurons_frequency_map_with_phase_color_" + sigla + "_" + str(t_initial_analysis) + '_' + str(t_final_analysis) + '_' + str(interval_dim) + "_id_from_0_to_1.html")
else:
    fig3.write_html(results_path+"frequency_map_with_phase_color_"+sigla+"_"+str(t_initial_analysis)+'_'+str(t_final_analysis)+'_'+str(interval_dim)+"_id_from_0_to_1.html")
