...
results_path="./interval_"+str(t_initial_analysis)+"_"+str(t_final_analysis)+"interval_dim_"+str(interval_dim)+"_fft/"
try:
...
if only_interneurons:
    ...
    last_neuron = 1000
try:
    with open(results_path + 'aphase_and_freq_neurons_from_' + str(first_neuron) + '_' + str(
    ...
    for id_neu in range(first_neuron,last_neuron):#n_neuroni):
        ...
        if coef_var[id_neu]>1:
            ...
            #
            #     j = j + 1
            coef_var_bet_periods[id_neu] = 0.2#np.std(ns) / np.mean(ns)
