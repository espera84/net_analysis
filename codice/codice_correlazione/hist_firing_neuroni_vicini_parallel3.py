...
def select_spk_for_voxel(list_id_neuroni):
    ...
    n_workers=10
    print("numero voxel = "+str(list_id_neuroni.__len__()))
...
def plot_prerdecessori(id,n_pred):
    ...
    fig.write_html(results_path + "hist_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
        neu_min_per_blocco_to_plot) + "voxel_" + str(
        posizioni_centri[id_center[block_plotted[id]], :]) + "_and_pred.html")
    fig_norm.write_html(results_path + "hist_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
        neu_min_per_blocco_to_plot) + "voxel_" + str(
        posizioni_centri[id_center[block_plotted[id]], :]) + "_and_pred_normalized.html")
    fig3.write_html(results_path + "positions_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
        neu_min_per_blocco_to_plot) + "voxel_" + str(
        posizioni_centri[id_center[block_plotted[id]], :]) + "_and_pred_pyr.html")
    fig4.write_html(results_path + "positions_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
        neu_min_per_blocco_to_plot) + "voxel_" + str(
...
results_path=results_path+"area_activity_dist_cent_"+str(bin_x)+"_voxel_sl_"+str(distanza_massima*2)+'_interval_'+str(t_initial_analysis)+"_"+str(t_final_analysis)+"/"
try:
    os.mkdir(results_path)
except FileExistsError:
    pass
...
try:
    with open(results_path + 'info_firing_per_area_dist_max_' + str(distanza_massima) +'_dist_cent_'+str(bin_x)+'_interval_'+str(t_initial_analysis)+"_"+str(t_final_analysis)+'.pkl', 'rb') as f:
    ...
    voxel_spk_int=voxel_spk_all[:, voxel_spk_all[1, :] > N_pyr]
    id_voxel_for_spk_int=id_voxel_for_spk_all[voxel_spk_all[1, :] > N_pyr]
    voxel_spk_pyr=voxel_spk_all[:, voxel_spk_all[1, :] <= N_pyr]
    id_voxel_for_spk_pyr=id_voxel_for_spk_all[voxel_spk_all[1,:]<=N_pyr]
    ...
    with open(results_path+'info_firing_per_area_dist_max_' + str(distanza_massima) +'_dist_cent_'+str(bin_x)+'_interval_'+str(t_initial_analysis)+"_"+str(t_final_analysis)+'.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
...
try:
    with open(results_path+'info_freq_per_voxel.pkl', 'rb') as f:
        [f,a,p,dft,fr,rad,perc_amp_max,period] = pickle.load(f)
except:
    ...
    f = np.empty((n_cubi), dtype=object)
    a = np.empty((n_cubi), dtype=object)
    p = np.empty((n_cubi), dtype=object)
    dft = np.empty((n_cubi), dtype=object)
#print('ao')
    for i in np.nditer(np.unique(id_voxel_for_spk.astype(int))):#range(id_center.__len__()):
        ...
        hst[i]=hist_value
        #print('ao1')
        [f[i], a[i], p[i],dft[i]] = fourier_analysis(hist_value)
        #print('ao2')
        fr[i] = f[i][np.argmax(a[i][1:]) + 1]
        rad[i] = p[i][np.argmax(a[i][1:]) + 1]
        perc_amp_max[i]=a[i][np.argmax(a[i][1:])+1]/a[i][1:].sum()
        #perc_amp_max_tot[i]=a[np.argmax(a[1:])]/a[1:].sum()
        period[i] = 1 / fr[i]

    with open(results_path+'info_freq_per_voxel.pkl', 'wb') as file:  # Python 3: open(..., 'wb')
        pickle.dump([f,a,p,dft,fr,rad,perc_amp_max,period] , file)
...
for i in np.nditer(np.unique(id_voxel_for_spk.astype(int))):
    dft[i][f[i] > buond_up_filter] = 0
    dft[i][f[i] < buond_down_filter] = 0
    filtered_sig[i]=np.fft.irfft(dft[i])
    rad_filtered[i]=p[i][np.logical_and(f[i] < buond_up_filter, f[i] > buond_down_filter)][0]
    #print(i)
    #print(hist_value.sum()/id_neurons_near_to_center[i].__len__())
    if (np.logical_and((hst[i].sum()/id_neurons_near_to_center[i].__len__())>min_spk,id_neurons_near_to_center[i].__len__()>neu_min_per_blocco_to_plot)):#hist_value.sum()>min_spk:#
        ...
        if first_clique:
            ...
            fig_norm = go.Figure(data=go.Scatter(
                # x=cl_spk[0, np.in1d(col, indici_cl)],
                x=(bins[1:] + bins[:-1]) / 2,
                y=hst[i]/id_neurons_near_to_center[i].__len__(),
                name='pyr_cube_' + str(posizioni_centri[id_center[i], :])+'_'+str(block_plotted.__len__()-1),
                # mode='markers',
                marker=dict(
                    # symbol=sim,
...
try:
    fig.write_html(results_path+"hist_firing_min_neu_spk_"+str(min_spk)+"_min_neu_Xb_"+str(neu_min_per_blocco_to_plot)+modality+".html")
    fig_filtered.write_html(results_path + "hist_firing_filtered_min_neu_spk_" + str(min_spk) + "_min_neu_Xb_" + str(neu_min_per_blocco_to_plot) + modality +"_"+str(buond_down_filter)+"_"+str(buond_up_filter)+ "_HZ.html")
    fig_norm.write_html(results_path+"hist_norm_firing_min_neu_spk_"+str(min_spk)+"_min_neu_Xb_"+str(neu_min_per_blocco_to_plot)+modality+".html")
    fig_filtered_norm.write_html(results_path + "hist_norm_firing_filtered_min_neu_spk_" + str(min_spk) + "_min_neu_Xb_" + str(neu_min_per_blocco_to_plot) + modality +"_"+str(buond_down_filter)+"_"+str(buond_up_filter)+ "_HZ.html")
except:
    pass
fig4.write_html(results_path+"position_centers_min_neu_spk_"+str(min_spk)+"_min_neu_Xb_"+str(neu_min_per_blocco_to_plot)+modality+"_col_freq.html")
fig6.write_html(results_path+"position_centers_th_perc_amp_"+str(threshold_perc_amp)+"_min_neu_spk_"+str(min_spk)+"_min_neu_Xb_"+str(neu_min_per_blocco_to_plot)+modality+"_col_freq.html")
fig7.write_html(results_path+"position_centers_th_perc_amp_"+str(threshold_perc_amp)+"_min_neu_spk_"+str(min_spk)+"_min_neu_Xb_"+str(neu_min_per_blocco_to_plot)+modality+".html")
...
fig7.write_html(results_path + "prominence_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
...
fig8.write_html(results_path + "phase_min_spk_" + str(min_spk) + "_min_neu_Xb_" + str(
        neu_min_per_blocco_to_plot) + modality+".html")
...
wave_direction=np.empty((block_plotted.__len__()),dtype=object)
wave_direction_norm=np.empty((block_plotted.__len__()),dtype=object)
inn_prod=np.empty((block_plotted.__len__()),dtype=float)
for i in range(block_plotted.__len__()):
    if vicini[i].__len__():
        wave_direction[i]=[posizioni_centri[np.array(id_center)[bp[vicini[i][vicino_max_phase_var[i]]]]][0] - posizioni_centri[np.array(id_center)[bp[i]]][0],posizioni_centri[np.array(id_center)[bp[vicini[i][vicino_max_phase_var[i]]]]][1] - posizioni_centri[np.array(id_center)[bp[i]]][1],posizioni_centri[np.array(id_center)[bp[vicini[i][vicino_max_phase_var[i]]]]][2] - posizioni_centri[np.array(id_center)[bp[i]]][2]]
        wave_direction_norm[i]=wave_direction[i]/np.linalg.norm(wave_direction[i])
...
fig9.write_html(results_path + "phase_path_filtered_between_" + str(buond_down_filter) + "_and_" + str(buond_up_filter) + ".html")
...
for i in range(block_plotted.__len__()):
    if vicini[i].__len__():
        ...
        inn_prod[i]=0
        for j in range(vicini[i].__len__()):
            inn_prod[i]=inn_prod[i]+np.inner(wave_direction_norm[i],wave_direction_norm[vicini[i][j]])
        inn_prod[i]=inn_prod[i]/vicini[i].__len__()
    else:
        inn_prod[i] = -100


fig7 = go.Figure(data=points, layout=layout)
fig7.add_scatter3d(x=posizioni_centri[np.array(id_center)[bp_w_v], 0],
                       y=posizioni_centri[np.array(id_center)[bp_w_v], 1],
                       z=posizioni_centri[np.array(id_center)[bp_w_v], 2],
                       mode='markers',
                       name='pyr_cube_',
                       text=inn_prod[inn_prod>-100],
                       marker=dict(size=5,
                                   colorscale='pinkyl',
                                   color=inn_prod[inn_prod>-100],
                                   # color_discrete_sequence[i],#px.colors.qualitative.Antique[i],#l,#list(colori),#px.colors.qualitative.Antique[i],#                                       color=px.colors.qualitative.Plotly[i],#float(i / n_comp_connesse),
                                   # colorscale='Plotly3',
                                   showscale=True,
                                   )
...
fig7.write_html(results_path + "Inner_prod_path_filtered_between_" + str(buond_down_filter) + "_and_" + str(buond_up_filter) + ".html")
