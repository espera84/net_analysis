...
for id_neu in range(first_neuron,last_neuron):#n_neuroni):
    ...
    if coef_var[id_neu]>1:
        ...
        ip_period = np.ones(hist.__len__()) * 10000
        bins=np.concatenate((iniz_intervals,[t_final_analysis]))
        for i in range(4, int(hist.__len__() / 4)):
            ip_period[i] = np.abs(hist[i:] - hist[:-i]).sum() / ((hist[i:].sum()+hist[:-i].sum())/2)#((hist.__len__()-i)*hist[i:].std())#

        periods[id_neu]=(time[ip_period.argmin()] - time[0])/1000
        fr[id_neu]=1/periods[id_neu]
        err_with_fr[id_neu]=ip_period.min()
        ip_fase = np.zeros(ip_period.argmin())
        for i in range(1, ip_period.argmin()):
            j = 0
            s = []
            while ((i + j * ip_period.argmin()) < hist.__len__()):
                s.append(hist[i + j * ip_period.argmin()] - hist[i + j * ip_period.argmin() - 1])
        ...
        ns=[]
        j=0
        while (((j+1) * ip_period.argmin()) < hist.__len__()):
            ns.append(hist[j * ip_period.argmin():(j+1) * ip_period.argmin()].sum())
