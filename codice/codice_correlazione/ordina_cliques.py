...
n_sim=3
ordine_clique=[[0,1,2,4,3],[0,1,4,3,2],[0,1,2,3]]
...
for i in range(n_sim):
    ...
    #indici_cl_aux=np.zeros(indici_cl[i].__len__())
    #for j in range(indici_cl[i].__len__()):
    #    indici_cl_aux[j]=indici_cl[i][ordine_clique[i][j]]
    indici_cl[i]=ordine_clique[i]
    col2 = col.copy()
    for j in range(indici_cl[i].__len__()):
    # indici_cl_aux[j]=indici_cl[i][ordine_clique[i][j]]
        col2[col==j]=ordine_clique[i][j]
    col=col2
    with open(results_sub_path[i] +  'clique_info.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([id_neu_cliques[i], indici_cl[i],cl_spk,col,Isi_cliques,perc_pyr] , f)