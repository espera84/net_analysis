...
import json
import sys
from compute_number_of_ini_ex_conn_prob import comp_ex_in_con
from calcola_prob_firing_neurons_all_parallel_2g import calcola_prob_2g
...
def calcola_pr_firing_dif(id_neuorn,fat_add):
    global n_con_tot_on,n_con_ex_on
...
with open('Connection_info_prob.pkl', 'rb') as f:
...
try:
    if modalità=="additiva":
        ...
        with open('Firing_Probability_all_mul_'+str(fat_molt)+'.pkl', 'rb') as f:
            [id,probabilità_firing, Combinazioni_tot, Combinazioni_pos] = pickle.load(f)
except:
...
#    Ltt = Parallel(n_jobs=n_workers, verbose=50)(delayed(calcola_pr_firing)(i,fat_molt) for i in range(N_pyr)) #prob firing considerando il rapporto tra sin eccitatorie e sin inibitorie
    Ltt = Parallel(n_jobs=n_workers, verbose=50)(delayed(calcola_pr_firing_dif)(i, fat_add) for i in range(N_neuroni)) #prob firing considerando la differenza tra sin eccitatorie e sin inibitorie
    ...
    if modalità=="additiva":
        with open('Firing_Probability_all_add_'+str(fat_add)+'.pkl', 'wb') as f:
            pickle.dump([id, probabilità_firing, Combinazioni_tot, Combinazioni_pos], f)
    else:
        with open(path+'Firing_Probability_all_mul_'+str(fat_molt)+'.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump([id,probabilità_firing, Combinazioni_tot, Combinazioni_pos], f)
...
if modalità=="additiva":
        fig2.write_html("theoretical_prob_firing_map_pyr_add_" +str(fat_add)+".html")
else:
        fig2.write_html("theoretical_prob_firing_map_pyr_mult_" + str(fat_molt) + ".html")
...
if modalità=="additiva":
        fig_all.write_html("theoretical_prob_firing_map_all_add_" +str(fat_add)+".html")
else:
        fig_all.write_html("theoretical_prob_firing_map_all_mult_" + str(fat_molt) + ".html")
#fig2.show()
...
if add_clique:
    ...
    if modalità == "additiva":
        fig2.write_html("theoretical_prob_firing_map_pyr_add_" + str(fat_add) + "with_CL_"+sigla+".html")
        fig_all.write_html("theoretical_prob_firing_map_all_add_" + str(fat_add) + "with_CL_" + sigla + ".html")
    else:
        fig2.write_html("theoretical_prob_firing_map_pyr_add_" + str(fat_add) + "with_CL_"+sigla+".html")
        fig_all.write_html("theoretical_prob_firing_map_all_add_" + str(fat_add) + "with_CL_" + sigla + ".html")
...
if plot_only_int:
    ...
    if add_clique:
        ...
        if modalità == "additiva":
            fig2.write_html("theoretical_prob_firing_map_int_add_" + str(fat_add) + "with_CL_" + sigla + ".html")
        else:
            fig2.write_html("theoretical_prob_firing_map_int_mult_" + str(fat_molt) + "with_CL_" + sigla + ".html")

comp_ex_in_con(fat_add,1)
calcola_prob_2g(fat_add)