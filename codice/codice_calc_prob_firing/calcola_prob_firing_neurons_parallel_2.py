...
def calcola_pr_firing(id_pyr,fat_molt):
    ...
    for k in range (int(np.floor(n_con_tot_on_pyr[id_pyr]/10)),n_con_tot_on_pyr[id_pyr]):
        ...
        #combinazioni in input con più stimoli da sinapsi eccitatorie che inibitorie
        for i in range(int(np.floor(k*fat_molt))+1,min(k,n_con_ex_on_pyr[id_pyr])): #almeno fat_molt delle k sinapsi devono essere eccitatorie
            if (k - i< n_con_in_on_pyr[id_pyr]): #verifico che ci siano almeno k-i syn inibitorie
...
def calcola_pr_firing_dif(id_pyr,fat_add):
    global n_con_tot_on_pyr,n_con_ex_on_pyr
...
with open('Pyr_connection_info.pkl', 'rb') as f:
...
try:
    if modalità=="additiva":
        with open('Firing_Probability_add_'+str(fat_add)+'.pkl', 'rb') as f:
            [id,probabilità_firing, Combinazioni_tot, Combinazioni_pos] = pickle.load(f)
    else:
        with open('Firing_Probability_'+str(fat_molt)+'.pkl', 'rb') as f:
            [id,probabilità_firing, Combinazioni_tot, Combinazioni_pos] = pickle.load(f)
except:

    N_pyr=10000#id.__len__()
...
#    Ltt = Parallel(n_jobs=n_workers, verbose=50)(delayed(calcola_pr_firing)(i,fat_molt) for i in range(N_pyr)) #prob firing considerando il rapporto tra sin eccitatorie e sin inibitorie
    Ltt = Parallel(n_jobs=n_workers, verbose=50)(delayed(calcola_pr_firing_dif)(i, fat_add) for i in range(N_pyr)) #prob firing considerando la differenza tra sin eccitatorie e sin inibitorie
    ...
    if modalità=="additiva":
        with open('Firing_Probability_add_'+str(fat_add)+'.pkl', 'wb') as f:
...
fig2.add_scatter3d(x = np.array(posizioni)[:10000,0],
                    y = np.array(posizioni)[:10000,1],
                    z = np.array(posizioni)[:10000,2],
                    mode = 'markers',
                    marker = dict( size = 1,
                                   colorscale='pinkyl',
                                   color=probabilità_firing,#np.random.rand(1)[0]n,#color_discrete_sequence[n],#n/10,
                                   showscale=True)
                    )  # ,showscale=True), )
...
if modalità=="additiva":
        fig2.write_html("theoretical_prob_firing_map_add_" +str(fat_add)+".html")
else:
        fig2.write_html("theoretical_prob_firing_map_mult_" + str(fat_add) + ".html")
#fig2.show()
